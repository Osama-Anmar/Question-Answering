...
class TextProcessing():
    ...
    def data_words(self, filter = None):
        if filter:
            self.data = self.filter_data(filter)
        words = set() 
    ...
    def filter_data(self, pattern):
        filtered_data = [re.sub(pattern, " ", txt) for txt in self.data]
    ...
    def text_to_sequence(self, word_index):
     sequence_text = [[word_index[word] if word in word_index else word_index["UNK"] for word in sentences.split()] for sentences in self.data]
    ...
    def word_sequence_to_text(self, index_to_words, sequence, all_words):
